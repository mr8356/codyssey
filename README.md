...
## 스피드런(권장 실행 순서)
1. `python main.py` 실행 후 메뉴 출력 확인
2. `2` 선택 -> 퀴즈 추가 -> 저장 메시지 확인
3. `3` 선택 -> 추가된 퀴즈 목록 확인
4. `1` 선택 -> 퀴즈 풀기 -> 결과/점수(정답률 %) 및 최고점수 갱신 여부 확인
5. `4` 선택 -> 최고 점수 확인
6. `5`로 종료 후 재실행 -> 추가 퀴즈/최고점수 유지 확인
7. `git log --oneline --graph` 실행 후 결과를 스크린샷으로 남기기

## Git 작업 전략(요구사항 반영)
- 의미 있는 커밋 최소 10개 이상(메뉴/Quiz/플레이/추가/저장/README 등)
- 기능 단위 커밋 메시지: `Feat: ...`, `Fix: ...`, `Docs: ...`, `Refactor: ...`
- 브랜치 생성 후 병합(checkout/merge) 기록 1회 이상
- `clone`, `pull`을 각각 최소 1회 이상 사용한 기록이 남도록 진행
